...
def bout_count(df_boutdf):
    ...
    #Take average of count throughout experiment days
    ...
    bout_count = bout_count_df.fillna(0)
    #Seperate condition
    bout_count = sep_condition(bout_count)
    return(bout_count)

#Create a function for counting bout lenghts
def bout_length_compiled(df_boutdf):
    #Take the average of bout_length throughout each date entry
    activity_mean = df_boutdf.groupby(['Channel', 'Condition'])['bout_length'].mean().to_frame().reset_index()
    Ind_activity_bout_nodead_compiled = activity_mean.fillna(0)
